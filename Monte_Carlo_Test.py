...
# CORE SIMULATION APP
# --------------------------------------------------------------------------------
# Use tabs for better organization of inputs vs results
tab1, tab2 = st.tabs(["📊 Simulation Setup", "📈 Results"])
...
# Add a notification after running simulation to navigate to Results tab
    if run_simulation:
        # Display a prominent notification in the Setup tab to direct users to the Results tab
        with tab1:
            st.success("""
            ✅ **Simulation Completed Successfully!**
            
            Your results are now available in the **Results** tab above. 
            Click on the "📈 Results" tab to view pressure drop analysis and statistics.
            """)
            
            # Add a button to navigate to results tab for better user experience
            if st.button("📊 View Results Now", use_container_width=True):
                # This button doesn't need functionality as clicking it will refresh the page
                # The notification is enough to guide users
                pass
...
if run_simulation:
    with tab2:
        ...
        mu_samples = mu_samples_mPa * 1e-3  # mPa·s => Pa·s
        ...
        with eng_cols[0]:
            st.metric("Reynolds Number", f"{np.mean(Re_samples):.3g}")
            re_icon = "🌊" if np.mean(Re_samples) > 4000 else "➡️"
            flow_regime = "Turbulent" if np.mean(Re_samples) > 4000 else "Transitional" if np.mean(Re_samples) > 2000 else "Laminar"
            st.markdown(f"Flow Regime: {re_icon} **{flow_regime}**")
            
        with eng_cols[1]:
            st.metric("Friction Factor", f"{np.mean(f_samples):.5g}")
            st.markdown(f"ε/D: **{np.mean(epsilon_samples/D_samples):.2g}**")
    ...
    tab2.write("Adjust parameters and click **Run Simulation** to begin.")
...
with st.expander("📐 View Equations Used", expanded=False):
    ...
    **Volumetric Flow Rate:**
    ...
    st.latex(r"f = \frac{64}{Re} \quad \text{for} \quad Re \leq 2000")
    
    st.markdown("**Head Loss (Darcy-Weisbach):**")
    st.latex(r"h_f = f\,\frac{L}{D}\,\frac{v^2}{2g}")
    
    st.markdown("**Head Loss from Fittings and Valves:**")
    st.latex(r"h_\mathrm{fittings} = K_\mathrm{total}\,\frac{v^2}{2g}")
    
    st.markdown("**Total Head Loss:**")
    st.latex(r"h_\mathrm{total} = h_f + h_\mathrm{fittings} + \Delta z")
    
    st.markdown("**Pressure Drop:**")
    
    st.latex(r"\Delta P = \rho\,g\,h_\mathrm{total}")